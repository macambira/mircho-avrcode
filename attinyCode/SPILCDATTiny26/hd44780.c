...
#warning This should not happen! Have a device.h file!

#define	IF_BUS		8			/* Data bus width (4 or 8) */
#define	IF_INIT()	{}			/* Initialize control port */
#define E1_HIGH()	bset(E)		/* Set E(E1) high */
#define E1_LOW()	bset(E)		/* Set E(E1) low */
#define E2_HIGH()				/* Set E2 high (dual controller only) */
#define E2_LOW()				/* Set E2 low (dual controller only) */
#define	RS_HIGH()	bset(RS)	/* Set RS high */
#define	RS_LOW()	bclr(RS)	/* Set RS low */
#define	OUT_DATA(d)	outp(Q, d);	/* Output a byte d on the data bus (higher 4 bits of d in 4-bit mode) */
#define	IF_DLY60()	{}			/* Delay >=60ns (can be blanked for most uC) */
#define	IF_DLY450()	{}			/* Delay >=450ns@3V, >=250ns@5V */
#define DELAY_US(n)	{}			/* Delay n microseconds */
...
#define	LCD_DLF		3.0			/* Delay factor (>=2.0) */
