...
static unsigned char ActiveDisplay=1;
  ...
  void shiftOut(uint8_t bitOrder, uint8_t val)
  {
    ...
    for (i = 0; i < 8; i++)  {
      ...
      //7-i == 0b111^i
      sh = ( bitOrder == LSBFIRST ) ? i : ( 0b111 ^ i );
      LCD_DATA = ( ( val & ( 1 << sh ) ) >> sh );
  ...
  #define SR_DDR_INIT() (LCD_SR_DDR |= (_BV(LCD_CLK_PIN)|_BV(LCD_DATA_PIN)))
  #define SR_BITMASK (SR_DATA_BIT0 | SR_DATA_BIT1 | SR_DATA_BIT2 | SR_DATA_BIT3)

  #define SR_upperNibble(value) ((value >> SR_MOVE_UN_BITS) & SR_BITMASK)
  #define SR_lowerNibble(value) ((value << SR_MOVE_LN_BITS) & SR_BITMASK)

  #define SR_output(value) shiftOut(MSBFIRST,SR_EN_BIT|value)
  #define SR_clear(value) shiftOut(MSBFIRST,0)
  #define SR_outputUpperNibble(value,rs) ( SR_output((rs?SR_RS_BIT:0)|SR_upperNibble(value)) )
  #define SR_outputLowerNibble(value,rs) ( SR_output((rs?SR_RS_BIT:0)|SR_lowerNibble(value)) )
  #define SR_outputBitmask() ( SR_output( SR_BITMASK ) )
  #define SR_outputClear() ( SR_clear() )

#endif

#if (LCD_SHIFT_REGISTER == 1)

  #define lcd_e_port_low() (LCD_DATA=0)
...
static inline void lcd_e_port_low()
{
  ...
  #define lcd_e_port_high() (LCD_DATA=1)
...
static void lcd_write(uint8_t data,uint8_t rs)
  {
    ...
    #if (WAIT_MODE==0 || RW_LINE_IMPLEMENTED==0)
      if (!rs)
		if( data & ( _BV(LCD_CLR) | _BV(LCD_HOME) ) ) // Is command clrscr or home?
			Delay_ms( 2 );
		else
			Delay_us( 1 );
      else Delay_us( 1 );
...
void lcd_init(void)
  {
    ...
    SR_DDR_INIT();
      ...
      SR_output( SR_DATA_BIT0 | SR_DATA_BIT1 );
      ...
      SR_outputClear();
      SR_output( SR_DATA_BIT1 | SR_DATA_BIT3 );
